(() => {
  ...
  const listFromResponse = data => Array.isArray(data) ? data : (Array.isArray(data?.results) ? data.results : (Array.isArray(data?.data) ? data.data : []));

  function render(data) {
    const items = listFromResponse(data);
    const firstRow = items[0] || {};
    const student = data?.student || firstRow.student || {};
    ...
    if(rows) rows.innerHTML = items.length ? items.map(r => {
      const subject = first(r,['subject_name','subject','name']) || 'Subject';
      ...
      return `<tr><td>${esc(subject)}</td><td>${esc(ca)}</td><td>${esc(exam)}</td><td><strong>${esc(score)}</strong></td><td><span class="grade-badge">${esc(grade)}</span></td><td>${esc(remark)}</td></tr>`;
  ...
  async function checkResult(event) {
    ...
    try {
      ...
      const items = listFromResponse(data);
      if(!items.length && !data?.student && !data?.result) throw new Error('No published result was found for those details.');
      render(data?.result || data);
      setMessage('Result verified successfully.','success');
    } catch(error) {
      setMessage(error.message || 'Unable to check the result right now. Please try again.','error');
